...
#Randomized SVD k=n
rsvd_out <- pcaone(testMat, k = n, p = 0, method = "rsvd")
...
#Randomized SVD k=k
rsvd_out <- pcaone(testMat, k = k, p = 0, s = 0, method = "rsvd")
testMat.re <- rsvd_out$u %*% diag(rsvd_out$d) %*% t(rsvd_out$v)
...
#Randomized SVD k=n
rsvd_out <- pcaone(testMat, k = n, p = 3, s = 10, method = "rsvd")
testMat.re = rsvd_out$u %*% diag(rsvd_out$d) %*% t(rsvd_out$v)
...
#Randomized SVD k=k
rsvd_out <- pcaone(testMat, k = k, p = 2, s = 0, method = "rsvd")
testMat.re = rsvd_out$u %*% diag(rsvd_out$d) %*% t(rsvd_out$v)
